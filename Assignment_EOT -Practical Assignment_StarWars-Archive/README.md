# Source Code Repository for [StarWars 资料站](http://sgqy.top)

***For 殷方言老师：Please read the sections below carefully before reviewing or grading.***

## About
- This repository contains exactly the same files as the [main source code repository](https://github.com/Misaka24/StarWars-Archive) for my website [StarWars 资料站](http://sgqy.top). 
- However this repository is only uesd to store the source code of the EOT practical assignment to meet the requirements of this course.
- PLEASE SEE [https://github.com/Misaka24/StarWars-Archive](https://github.com/Misaka24/StarWars-Archive) FOR THE FUUL DETAILS.

~~## Deployment ~~
~~- This website is automatically deployed by GitHub Actions when the code is pushed to this repository. ~~

## Features
- For a full feature list of the website and this repository, see [Features.md](https://github.com/Misaka24/StarWars-Archive/Features.md).

## Future Work
- For the future work of the website, see [FutureWork.md](https://github.com/Misaka24/StarWars-Archive/FutureWork.md).
